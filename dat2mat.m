...
% Adapted by Jos� A. P�rez-Carrasco in 2007, while at the Sevilla
...
if ~isempty(kk)
    if max(allTs) >  intmax('int32')
        for i=1:length(kk)-1
            tpo(kk(i)+1:kk(i+1))=tpo(kk(i))*int64(ones(size(tpo(kk(i)+1:kk(i+1))))) + tpo(kk(i)+1:kk(i+1));
        end

        tpo(kk(end)+1:end)=tpo(kk(end))*int64(ones(size(tpo(kk(end)+1:end)))) + tpo(kk(end)+1:end);
    elseif tpo(kk) <= tpo(kk + 2)
        tpo(kk + 1) = tpo(kk);
    else
        for i=1:length(kk)-1
